...
class Facebook {
  ...
  getUsernameId() {
    const options = {
      url: `https://graph.facebook.com/v2.7/${this.username}?access_token=${this.config.token}`
    };
  ...
  getPostInfo() {
    const options = {
      url: `
        https://graph.facebook.com/v2.7/${this.userId}_${this.id}?access_token=${this.config.token}&fields=name,description,picture`
    };
